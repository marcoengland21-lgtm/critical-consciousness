...
--
-- This is the FILLED version with actual UUIDs substituted from the
-- production Supabase Auth users (per the screenshot you shared):
--   Mars       : 97d2db21-64af-42e9-8765-d7ae30518c17
--   user_alpha : fd5320d4-a311-4b2c-ab10-3aa615bde326
--   user_beta  : 46964cf3-8baa-4b00-a73d-deac084090af
--
...
--
-- HOW TO RUN: paste the entire file into the SQL editor and click Run.
-- The editor will show results for each SELECT separately and the
-- INSERT errors inline. Each impersonation is wrapped in BEGIN/ROLLBACK
-- so a partial run can't leak state — and so the cross-group INSERT
-- attempts (which SHOULD fail) don't pollute the database.
--
-- IF the editor only shows the LAST statement's result and you can't
-- see the cross-group SELECT counts: run the blocks one at a time
-- (highlight the block, click Run on highlighted text). Block 5b
-- first (alpha SELECTs + alpha INSERT), then block 5c, then 5d.
-- ====================================================================
...
BEGIN;
SET LOCAL ROLE authenticated;
SET LOCAL "request.jwt.claim.sub" = '97d2db21-64af-42e9-8765-d7ae30518c17';
