...
export interface TiptapEditorProps {
  ...
  /** Called once when the editor is created — gives the parent a handle.
      The parent uses this to drive the toolbar (lifted from test-news where
      the toolbar lives at the parent level alongside the editor). */
...
/**
 * TiptapEditor — the journal writing surface. Renders ONLY the editor area,
 * not the toolbar. The toolbar is rendered by the parent (JournalEditor) so
 * it can also display title + save-status state — same pattern as test-news's
 * `WriterStudioLayout` rendering `<CompactToolbar>` next to `<WriterEditor>`.
 *
  ...
  return (
    <div className={className} style={{ position: 'relative' }}>
      ...
      {/* Stats footer — exposed in the parent via onStatsChange but also
          rendered here for convenience. Hidden by default; the parent
          renders its own footer with the status indicator. */}
